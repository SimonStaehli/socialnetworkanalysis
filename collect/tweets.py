...
class HashForATweet:
    ...
    def __init__(self, authentication,
                 ...
                 search_results='recent', language='en', since_id):
        """
    ...
    def collect_tweets(self):
        ...
        for status in tweepy.Cursor(self.api.search, q=self.search_key, lang=self.language, tweet_mode='extended',
                                    ...
                                    since_id=self.id_max).items(self.amount_tweets):
            try:
            ...
            self.id_max = max([tweet['tweet_id'] for tweet in self.tweets])
    ...
    def write_tweets_csv(self):
        ...
        df.to_csv(path_or_buf=f'../data/{datetime.datetime.now().strftime(format='%y-%m-%d_%H:%M:%S')}_{self.search_key}.csv')
